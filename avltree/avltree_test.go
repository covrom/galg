...
func TestAVLTreePut(t *testing.T) {
	tree := New[int, string](IntComparator)
	tree.Put(5, "e")
	tree.Put(6, "f")
	tree.Put(7, "g")
	tree.Put(3, "c")
	...
	if actualValue, expectedValue := fmt.Sprintf("%d%d%d%d%d%d%d", AnySlice(tree.Keys())...), "1234567"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s%s%s%s%s", AnySlice(tree.Values())...), "abcdefg"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
...
func TestAVLTreeRemove(t *testing.T) {
	...
	if actualValue, expectedValue := fmt.Sprintf("%d%d%d%d", AnySlice(tree.Keys())...), "1234"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s%s", AnySlice(tree.Values())...), "abcd"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	if actualValue, expectedValue := fmt.Sprintf("%s%s%s%s", AnySlice(tree.Values())...), "abcd"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	...
	if actualValue, expectedValue := fmt.Sprintf("%s", AnySlice(tree.Keys())), "[]"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
...
func TestAVLTreeIterator4Next(t *testing.T) {
	tree := New[int, int](IntComparator)
	tree.Put(13, 5)
	tree.Put(8, 3)
	tree.Put(17, 7)
	tree.Put(1, 1)
...
func TestAVLTreeIteratorBegin(t *testing.T) {
	...
	if ik, ok := it.Key(); ok {
		t.Errorf("Got %v expected %v", ik, nil)
	}
...
func TestAVLTreeSerialization(t *testing.T) {
	tree := New[string, string](StringComparator)
	tree.Put("c", "3")
	tree.Put("b", "2")
	tree.Put("a", "1")
...
func BenchmarkAVLTreeGet100(b *testing.B) {
	b.StopTimer()
	size := 100
	tree := New[int, struct{}](IntComparator)
	for n := 0; n < size; n++ {
		tree.Put(n, struct{}{})
	}
	b.StartTimer()
